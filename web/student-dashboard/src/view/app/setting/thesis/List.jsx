...
export default function List() {
    ...
    const [inputGeneration, setInputGeneeration] = React.useState('');
    ...
    const handleView = async (thesis_id) => {
        await axios
            ...
            .then((result) => {
                ...
                setInputFile(result.data[0].file)
    ...
    const handleSubmitEdit = async () => {
        ...
        for (var i = 0; i < length; i++) {
            const response1 = await axios.post(
                'http://localhost:3000/' + 'admin/verify/course/schedule',
                {
                    room: Room[i],
    ...
    const handleSubmit = async () => {
        ...
        formData.append('student_id', inputID);
        formData.append('supervisor_name', inputTeacher_name);
        ...
        formData.append('year', inputGeneration);
    ...
    const handleDelete = async () => {
        axios.post('http://localhost:3001/thesis/remove' + deleteID)
            .then((result) => {
                window.location.replace('/thesis/list');
            })
            .catch((error) => console.log(error));
                    ...
                    <Grid templateColumns="repeat(4,1fr)  " gap="2">
                        <VStack spacing="3">
                            <FormControl sx={{ width: '300px' }}>
                                ...
                                <SELECT_OPTIONS
                                    onChange={(e) => setInputTeacher_name(e.target.value)}
                                    placeholder="Teacher"
                                    // defaultValue={[inputTeacher_id[20], inputTeacher_id[20]]}
                                    options={inputTeacher_name}
                                ></SELECT_OPTIONS>
